...
text_renderer::Model::Model(const char* path) { loadModel(path); }

auto text_renderer::Model::Draw(const Program& program) const -> void {
  for (auto mesh : meshes) {
    mesh.Draw(program);
  }
}
auto text_renderer::Model::loadModel(const std::string& path) -> void {
...
auto text_renderer::Model::processNode(aiNode* node, const aiScene* scene)
    -> void {
...
auto text_renderer::Model::processMesh(aiMesh* mesh, const aiScene* scene)
    -> Mesh {
...
auto text_renderer::Model::loadMaterialTextures(aiMaterial* mat,
                                                aiTextureType type,
                                                std::string typeName)
    -> std::vector<Texture> {
  std::vector<Texture> textures;
