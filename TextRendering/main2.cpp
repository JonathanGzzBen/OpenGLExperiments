...
i.e., rendering a huge number of quads in one batch rather than once for every
quad.

The following demostration doesn't use Element Buffers for the sake of
simplicity, the same can be extended to save a little bit of VRAM

The following file is divided into following sections, search the following
keywords to reach the following sections:
...
// A struct to hold all the useful stuff. Used this instead declaring them
// globally.
struct LocalState {
  GLFWwindow* window;
...
static void SetupShaderProgram(const char* vertexShaderSource,
                               const char* fragmentShaderSource);
static void SetupFontTexture(uint32_t fontAtlasWidth, uint32_t fontAtlasHeight,
                             void* textureData, size_t size);
...
static uint8_t* SetupFont(const std::string& fontFile) {
  ...
  stbtt_PackFontRange(
      ...
      0,            // Font Index
      fontSize,  // Size of font in pixels. (Use STBTT_POINT_SIZE(fontSize) to
                 // use points)
      codePointOfFirstChar,       // Code point of the first charecter
      charsToIncludeInFontAtlas,  // No. of charecters to be included in the
                                  // font atlas
      localState.packedChars      // stbtt_packedchar array, this struct will
                                  // contain the data to render a glyph
  );
  stbtt_PackEnd(&ctx);
  ...
  for (int i = 0; i < charsToIncludeInFontAtlas; i++) {
    ...
    stbtt_GetPackedQuad(
        ...
        &localState
             .alignedQuads[i],  // stbtt_alligned_quad struct. (this struct
                                // mainly consists of the texture coordinates)
  ...
  stbi_write_png("fontAtlas.png", fontAtlasWidth, fontAtlasHeight, 1,
                 fontAtlasTextureData, fontAtlasWidth);
...
static void RendererInit() {
  ...
  SetupFontTexture(fontAtlasWidth, fontAtlasHeight, fontTextureData,
                   fontAtlasWidth * fontAtlasHeight * sizeof(uint8_t));
...
static void DrawBegin() { localState.vertexIndex = 0; }
...
static void DrawText(const std::string& text, glm::vec3 position,
  ...
  for (char ch : text) {
    ...
    if (ch >= codePointOfFirstChar &&
        ch <= codePointOfFirstChar + charsToIncludeInFontAtlas) {
      ...
      stbtt_packedchar* packedChar =
          &localState.packedChars[ch - codePointOfFirstChar];
      stbtt_aligned_quad* alignedQuad =
          &localState.alignedQuads[ch - codePointOfFirstChar];
      ...
      glm::vec2 glyphBoundingBoxBottomLeft = {
          ...
          localPosition.y -
              (packedChar->yoff + packedChar->y1 - packedChar->y0) *
                  pixelScale * size};
      ...
      for (int i = 0; i < 6; i++) {
        localState.vertices[localState.vertexIndex + i].position =
            glm::vec3(glyphVertices[order[i]], position.z);
        localState.vertices[localState.vertexIndex + i].color = color;
        localState.vertices[localState.vertexIndex + i].texCoord =
            glyphTextureCoords[order[i]];
      }

      localState.vertexIndex += 6;

      // Update the position to render the next glyph specified by
      // packedChar->xadvance.
...
const size_t VBO_SIZE =
    600000 * sizeof(Vertex);  // Size enough for 600000 vertices (100000 quads)
...
static void SetupFontTexture(uint32_t textureWidth, uint32_t textureHeight,
                             void* textureData, size_t size) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_R8, textureWidth, textureHeight, 0, GL_RED,
...
static void BindFontTexture(uint8_t slot) {
  ...
  int uniformLoc =
      glGetUniformLocation(localState.shaderProgramID, "uFontAtlasTexture");
...
static void Render(const std::vector<Vertex>& vertices) {
  ...
  for (int i = 0; i < drawCallCount; i++) {
    ...
    int uniformLocation =
        glGetUniformLocation(localState.shaderProgramID, "uViewProjectionMat");
    glUniformMatrix4fv(uniformLocation, 1, GL_TRUE,
                       glm::value_ptr(localState.viewProjectionMat));
...
// GLFW WINDOW CODE:
// -------------------------------------------------------------------------------------------------------------
static void SetupWindowAndContext(uint32_t windowWidth, uint32_t windowHeight,
  ...
  // Create the window
  localState.window = glfwCreateWindow(windowWidth, windowHeight, title.c_str(),
                                       nullptr, nullptr);
...
int main() {
  ...
  while (!WindowShouldClose()) {
    ...
    DrawText("stb_truetype.h example", {-0.8f, 0.4f, 0.0f},
             {0.9f, 0.2f, 0.3f, 1.0f}, 1.0f);
    DrawText(textToDisplay, {-1.0f, 0.0f, 0.0f}, {1.0f, 1.0f, 1.0f, 1.0f},
             0.7f);
    DrawText("The color of text can be changed too!", {-0.5f, -0.4f, 0.0f},
             {0.1f, 0.5f, 1.0f, 1.0f}, 0.5f);
    DrawText("This is a \nNewline demo", {-0.9f, -0.7f, 0.0f},
             {0.7f, 0.8f, 0.0f, 1.0f}, 0.6f);
