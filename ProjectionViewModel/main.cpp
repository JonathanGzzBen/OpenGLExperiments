...
auto main() -> int {
  ...
  const auto projection_mat =
      glm::perspective(glm::radians(45.0F), 1.33F, 0.1F, 10.0F);
  ...
  // View Matrix
  const auto view_matrix = glm::translate(glm::mat4(1.0F), glm::vec3(0.0F, 0.0F, -2.5F));
  ...
  const auto scale_matrix =
      glm::scale(glm::mat4(1.0), glm::vec3(0.5F, 0.5F, 1.0F));
  const auto model_matrix =
      glm::rotate(scale_matrix, 1.57F, glm::vec3(0.0F, 0.0F, 1.0F));
