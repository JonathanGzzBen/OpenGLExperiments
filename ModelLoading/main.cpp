...
auto load_model(const std::string& path) -> std::vector<Vertex> {
...
auto main() -> int {
  ...
  const auto vertices = load_model("models/spider.obj");
  glNamedBufferStorage(vbos[0], vertices.size() * sizeof(Vertex), vertices.data(), 0);
